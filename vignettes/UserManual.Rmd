...
header-includes:
  ...
  - \makeatother
indent: true
knit: (function(inputFile, encoding, output_dir) { rmarkdown::render(inputFile, encoding = encoding, output_file ='README.md') })
vignette: >
...
```{r, fig.cap="SV-plot produced from a `GEVASummary` object using the `geva.summarize` function with default parameters."}
plot(gsummary)
```
...
```{r, fig.cap="SV-plot produced from a `GEVAQuantiles` object using the `geva.quantiles` function with default parameters."}
plot(gquants)
```
...
```{r, echo=FALSE, fig.cap="SV-plot produced from a `GEVAQuantiles` object using the `geva.quantiles` function using the `initial.thresholds = c(S=1, V=0.5)` parameter."}
if (!exists("gquants2"))
...
```{r, fig.cap="SV-plot produced from a `GEVACluster` object using the `geva.cluster` function with the hierarchical method and 30% of resolution."}
plot(gcluster)
```
...
```{r, echo=FALSE, fig.cap="SV-plot produced from a `GEVAGroupedSummary` object after appending the `GEVAQuantiles` and `GEVACluster` objects from previous steps."}
...
```{r, fig.cap="SV-plot produced from a `GEVAResults` object using the `geva.finalize` function with `0.05` as p-value cutoff."}
plot(gresults)
```
